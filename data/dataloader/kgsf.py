...
class KGSFDataLoader(BaseDataLoader):
    ...
    Notes:
        You can set the following parameters in config:   在“config”中可以设置以下参数

        - ``'context_truncate上下文截断'``: the maximum length of context.
        - ``'response_truncate响应截断'``: the maximum length of response.
        ...
        the above values specify the id of needed special token.上面的值指定所需的特殊令牌的id。

        - ``'n_entity'``: the number of entities in the entity KG of dataset.数据集的实体KG中的实体数
    ...
    def pretrain_batchify(self, batch):
        ...
        for kkk in range(len(batch)):
            conv_dict = batch[kkk]
            batch_user_id.append(int(conv_dict['user_id']))
        ...
        for conv_dict in tqdm(self.dataset):
            if conv_dict['role'] == 'Recommender':
                for movie in conv_dict['items']:
                    ...
                    augment_conv_dict['user_id'] = conv_dict['user_id']
