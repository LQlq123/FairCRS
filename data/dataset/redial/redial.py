...
class ReDialDataset(BaseDataset):
    ...
    def __init__(self, opt, tokenize, restore=False, save=False):
        ...
        import json
        with open("data/dataset/redial/nltk/train_data.jsonl", encoding="UTF-8") as f:
            raw_data = [json.loads(line) for line in f]
        with open("data/dataset/redial/nltk/test_data.jsonl", encoding="UTF-8") as f:
            raw_data += [json.loads(line) for line in f]
        self.raw_data = raw_data
        self.conv_raw = {conv["conversationId"]: conv for conv in self.raw_data}
    ...
    def _data_preprocess(self, train_data, valid_data, test_data):
        ...
        # Testing model performance for different user groups

        # user_g = "data/dataset/redial/nltk/group/active5.json"
        # user_g = "data/dataset/redial/nltk/group/inactive5.json"
        # with open(user_g, 'r', encoding='utf-8') as file:
        #     user_g = json.load(file)
        # _processed_test_data = []
        # for conv in processed_test_data:
        #     user_id = conv.get("user_id")
        #     if user_id in user_g:
        #         _processed_test_data.append(conv)
        # processed_test_data = _processed_test_data
    ...
    def _raw_data_process(self, raw_data):
        ...
        for conv, raw_conv in tqdm(zip(augmented_convs, raw_data)):
            augmented_conv = self._augment_and_add(conv)
            for aug_conv in augmented_conv:
                user_id = self.conv_raw[raw_conv["conv_id"]]["initiatorWorkerId"]
                aug_conv["user_id"] = user_id
            augmented_conv_dicts.extend(augmented_conv)
